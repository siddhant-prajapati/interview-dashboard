...
3. **Maven 3.9+** (Optional: You can use the pre-packaged `./mvnw` or `mvnw.cmd` inside `interview-dashboard-backend`).
...
Navigate into `interview-dashboard-backend/src/main/resources/application.yaml`:
...
> [!TIP]
> If your MySQL `root` password is not `root`, update the `password` field in `application.yaml` or pass it via system environment variables.
